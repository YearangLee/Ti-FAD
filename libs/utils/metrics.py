...
def remove_duplicate_and_short_annotations(ants, tol=1e-3, eps=0.01):
    # remove duplicate annotations (same category and starting/ending time)
    valid_events = []
    for event in ants:
        s, e, l = event['segment'][0], event['segment'][1], event['label_id']
...
def remove_subset_labels(ants, label_dict):
    valid_events = []
    for event in ants:
        # remove labels 
        exists_subset_labels = False
        if label_dict is not None:
            if event['label'] in label_dict.keys():
                exists_subset_labels = True
        if exists_subset_labels:
            valid_events.append(event)

    return valid_events

def load_gt_seg_from_json(json_file, split=None, label='label_id', label_offset=0, label_dict=None):
    # load json file
    ...
    for k, v in json_db.items():
        ...
        ants = remove_duplicate_and_short_annotations(v['annotations'])

        if label_dict is not None:
            ants = remove_subset_labels(ants, label_dict)
        ...
        for event in ants:
            ...
            if label_dict is not None:
                label_id = label_dict[event['label']]
            elif isinstance(event[label], (Tuple, List)):
...
def save_predictions_to_json(preds, file_path):
    """
    Save the predictions DataFrame to a JSON file.

    Parameters:
    preds (pd.DataFrame): DataFrame containing the predictions to be saved.
    file_path (str): Path where the JSON file will be saved.
    """
    # Ensure the DataFrame is in the correct format
    if not isinstance(preds, pd.DataFrame):
        raise ValueError("The predictions must be a pandas DataFrame.")

    # Convert DataFrame to JSON format
    preds_json = preds.to_dict(orient='records')

    # Structure for JSON output
    json_output = {'database': {}}
    for idx, pred in enumerate(preds_json):
        video_id = pred['video-id']
        if video_id not in json_output['database']:
            json_output['database'][video_id] = []
        json_output['database'][video_id].append({
            'segment': [pred['t-start'], pred['t-end']],
            'label_id': pred['label'],
            'scores': pred['score']
        })
    # Save to JSON file
...
class ANETdetection(object):
    ...
    def __init__(
        ...
        with open(subset_file, "rt") as f:
            label_dict = {l[:-1]: idx for idx, l in enumerate(f.readlines())}
        self.ground_truth = load_gt_seg_from_json(
            ant_file, split=self.split, label=label, label_offset=label_offset, label_dict=label_dict)
    ...
    def compute_recall_at_n(self, ground_truth, prediction, n, tiou_thresholds=np.linspace(0.1, 0.5, 5)):
        """
        Compute recall at N proposals per video.
        ...
        prediction : pd.DataFrame
            Predicted segments and their scores.
        n : int
            Number of top-scored proposals to consider for each video.
        tiou_thresholds : np.array
            Array of tIoU thresholds.

        Returns:
        -------
        recall : np.array
            Recall values for each tIoU threshold.
        """
        recall = np.zeros(len(tiou_thresholds))
        total_gt = len(ground_truth)
        
        if total_gt == 0:  # Avoid division by zero
            return recall
        
        # Sort predictions by scores in descending order
        prediction = prediction.sort_values(by='score', ascending=False)
        
        for i, tiou_thr in enumerate(tiou_thresholds):
            selected_proposals = 0
            matched_gt = 0
            
            # Group by video-id
            pred_gbvn = prediction.groupby('video-id')
            
            for video_id, group in pred_gbvn:
                # Select top N predictions for current video
                top_n_preds = group.head(n)
                
                # Get ground truth for current video
                gt_for_video = ground_truth[ground_truth['video-id'] == video_id]
                
                for _, pred_row in top_n_preds.iterrows():
                    # Compute temporal IoU with ground truth segments
                    tiou = segment_iou(pred_row[['t-start', 't-end']].values, 
                                        gt_for_video[['t-start', 't-end']].values)
                    
                    if any(tiou >= tiou_thr):
                        matched_gt += 1
                    selected_proposals += 1
            
            # Compute recall for the current tIoU threshold
            recall[i] = matched_gt / float(total_gt)
        
        return recall

    def wrapper_compute_recall_at_n(self, preds, n):
        """
        Wrapper function to compute recall at N proposals for each tIoU threshold.
        ...
        Returns:
        -------
        recall_at_n : np.array
            Recall values at N proposals for each tIoU threshold.
        """
        recall_at_n = self.compute_recall_at_n(self.ground_truth, preds, n, self.tiou_thresholds)
        return recall_at_n
    ...
    def evaluate(self, preds, verbose=True):
        ...
        # Compute AR@AN
        average_AR_5 = self.wrapper_compute_recall_at_n(preds, 5).mean()
        average_AR_10 = self.wrapper_compute_recall_at_n(preds, 10).mean()
        average_AR_20 = self.wrapper_compute_recall_at_n(preds, 20).mean()
        average_AR_30 = self.wrapper_compute_recall_at_n(preds, 30).mean()

        tiou_index = np.where(np.isclose(self.tiou_thresholds, 0.5))[0][0]  # Get index of tIoU = 0.5
        class_wise_ap = self.ap[tiou_index, :]  # AP values for each class at tIoU = 0.5
        ...
        if verbose:
            ...
            for tiou, tiou_mAP in zip(self.tiou_thresholds, mAP):
                ...
                tiou_mAP_list.append(round(tiou_mAP*100, 1))
            print(block)
            print('Avearge mAP: {:.2f} (%)'.format(average_mAP*100))
        
        return mAP, average_mAP, tiou_mAP_list
